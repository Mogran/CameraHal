...
* @file      camera_hal.c
* @brief     camera_hal.c
* @details   Hal files for rk3288 UVC Camera device
* @author    hjimi 
* @date      2018.5,5
* @version   1.0 
* @par Copyright (c): www.hjimi.com 
* @par History:          
*   1.0: lumingliang, 2018.5,5, first version\n 
*/ 

#include "camerahal.h"

camerahal_error_t camerahal_open(const char *videonode, camerahal_device_t *camera)
{
	int fd = 0;
...
camerahal_error_t camerahal_get_fmtdesc(camerahal_device_t *camera)
{
	...
	while( ioctl(camera->camerafd, VIDIOC_ENUM_FMT, &camera->fmtdesc) != -1){
		CAMERAHAL_DEBUG("   %d.%s ", camera->fmtdesc.index + 1, camera->fmtdesc.description);
...
/*
 *@brief    : 获取当前帧的分辨率、像素存储格式、颜色空间等参数
 *@param[0] :
 *@param[1] :
 *@param[2] :
 *@param[3] :
		...
		int count)
{
	...
	camera->buffers = (struct buffer*)calloc(camera->req.count, sizeof(*camera->buffers));		
